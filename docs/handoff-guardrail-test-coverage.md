...
| Handoff packet decision section | `scripts/quick_handoff_packet.py`, `tests/test_quick_handoff_packet.py` | Render the intraday decision, wait/confirmation conditions, and invalidation/exclusion conditions inside the review packet. |
...
| Quote and snapshot availability | `scripts/handoff_orchestrator.py`, handoff orchestrator tests | Force conservative `대기` for missing quote or snapshot fields unless documented hard invalidation requires `제외`. |
