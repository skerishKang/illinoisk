...
def test_parse_summary_only_markdown_fallback():
    """요약형 일지 포맷 fallback 파싱 테스트"""
    print("\n테스트 10: 요약형 일지 포맷 fallback 파싱")

    content = """# 2026-06-10 장중 대화 기록
...
### 1. 장세 분석 및 매매 논의
- **오늘 장세**: 삼성전자 -8.15%, SK하이닉스 -9.66% 폭락
- **매매 결과**: 없음 (관망)
...
"""
    ...
    assert topic == "📋 대화 핵심 요약", f"topic 불일치: {topic}"
    assert len(messages) == 1, f"메시지 수 불일치: {len(messages)}"
    assert messages[0]['speaker'] == "Agent", f"요약형 fallback 화자 불일치: {messages[0]['speaker']}"
    assert "두산테스나 매수 신호 놓침" in messages[0]['message'], "요약 내용 누락"
    assert "session_search" in messages[0]['message'], "세션 정보 누락"

    print("  ✓ 요약형 일지를 Agent 메시지 1건으로 fallback 파싱")
    print("  ✓ topic 추출 정상")
    print("  ✓ 본문/세션 정보 보존")
    return True
...
def run_all_tests():
    ...
    tests = [
        ...
        test_parse_summary_only_markdown_fallback,
    ]
